...
def compute_type_info(vartype, _type_info, contract):
    ...
    if type_ == "bool":
        ...
        # pprint(arraytype)
        if hasattr(arraytype, "is_fixed_array") or (arraytype.length is not None):
            base = arraytype.type 
...
def main_impl(sol_file, contractName, compilerVersion, outStorageFile=None):
    import os 
    # if os.path.exists(outStorageFile):
    #     return json.load(open(outStorageFile))
...
import argparse
def main():
    parser = argparse.ArgumentParser(description='Storage Layout File Generator!')
    parser.add_argument("--file", type=str, help="contract file (.sol)")
    parser.add_argument("--contractName", type=str, help="target contract name")
    parser.add_argument("--compilerVersion", type=str, help="compiler version")
    parser.add_argument("--ofile", type=str, help="output file (.json)", default=None)
    # assert len(sys.argv[1:])==4, \
    #     "file contractName compilerVersion. A four-items tuple on source code file (.sol), target contract name and compiler version, outputfile"
    # sol_file, compilerVersion, contractName, outStorageFile  = sys.argv[1:]
    # contractName = sys.argv[2]
    # compilerVersion = sys.argv[3]
    args = parser.parse_args()
    sol_file, compilerVersion, contractName, outStorageFile = args.file, args.contractName, args.compilerVersion, args.ofile
    contractStorageLayout = main_impl(sol_file=sol_file, contractName=contractName, compilerVersion=compilerVersion, outStorageFile=outStorageFile)
    print(contractStorageLayout)
